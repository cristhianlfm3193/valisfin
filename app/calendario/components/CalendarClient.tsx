...
export function CalendarClient({ initialEvents, currentMonth, currentYear }: CalendarClientProps) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden">
      {/* Header */}
      <header className="bg-[#121c27] border-b border-white/5 px-6 py-4 flex-shrink-0">
        ...
        <div className="mt-4 pt-3 border-t border-white/5 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-4 bg-white/10/90 p-1.5 rounded-2xl border border-white/10/60 shadow-sm relative overflow-hidden w-full lg:w-auto justify-between lg:justify-start">
            ...
            <div className="flex items-center">
              <button onClick={() => navigate(-1)} className="p-2 hover:bg-[#121c27] rounded-xl text-slate-400 hover:text-white transition shadow-xs">
            ...
            <button onClick={navigateToToday} className="px-4 py-2 text-xs font-bold text-slate-300 bg-[#121c27] border-2 border-white/10 rounded-xl hover:bg-brand-50 hover:border-brand-200 hover:text-brand-700 transition hidden sm:block">
              Hoy
            </button>
          </div>
          ...
          <div className="flex items-center bg-white/10/90 p-1.5 rounded-2xl border border-white/10/60 text-xs font-bold shadow-sm w-full lg:w-auto">
            ...
            <button 
              ...
              className={`flex-1 lg:flex-none px-4 py-2 rounded-xl transition-all ${viewType === 'Mes' ? 'bg-[#121c27] shadow-sm text-brand-700 ring-1 ring-black/5' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-200/50'}`}>Mes</button>
            <button 
              onClick={() => setViewType('Día')} 
              className={`flex-1 lg:flex-none px-4 py-2 rounded-xl transition-all ${viewType === 'Día' ? 'bg-[#121c27] shadow-sm text-brand-700 ring-1 ring-black/5' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-200/50'}`}>Día</button>
          </div>
        </div>
      </header>
      ...
      <div className="p-6 space-y-6 flex-1 overflow-y-auto bg-white/5/50">
        ...
        {/* Filters */}
        <section className="bg-[#121c27] p-3.5 rounded-2xl border border-white/10/70 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
            ...
            {Object.keys(CATEGORY_STYLES).map(cat => (
              <button 
                ...
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full font-semibold border transition whitespace-nowrap
                  ${filter === cat ? CATEGORY_STYLES[cat as keyof typeof CATEGORY_STYLES].bg + ' ' + CATEGORY_STYLES[cat as keyof typeof CATEGORY_STYLES].text + ' ' + CATEGORY_STYLES[cat as keyof typeof CATEGORY_STYLES].border : 'bg-[#121c27] text-slate-400 border-white/10 hover:bg-white/5'}`}
              >
        ...
        {viewType === 'Mes' && (
          <section className="bg-[#121c27] rounded-2xl border border-white/10/80 shadow-sm overflow-hidden flex flex-col">
            <div className="grid grid-cols-7 border-b border-white/10 text-center py-2.5 bg-white/5/70 text-xs font-bold uppercase tracking-wider text-slate-500">
              <div>Lun</div><div>Mar</div><div>Mié</div><div>Jue</div><div>Vie</div><div className="text-slate-300">Sáb</div><div className="text-slate-300">Dom</div>
            </div>
            
            <div className="grid grid-cols-7 border-b border-white/5 divide-x divide-y divide-white/5 flex-1">
              {days.map((d, i) => {
                ...
                return (
                  <div key={i} onClick={() => d.fullDate && setSelectedDay(d.fullDate)} className={`min-h-[100px] md:min-h-[120px] p-2 flex flex-col justify-between transition-colors cursor-pointer
                    ${!d.isCurrentMonth ? 'bg-white/5/40 text-slate-400' : 'hover:bg-white/5/50 text-slate-300'}
                  ...
                  `}>
                    <div className="flex justify-between items-start">
                      {isToday ? (
                        ...
                        <span className={`text-xs font-bold ${d.isCurrentMonth ? 'text-slate-300' : 'text-slate-400'}`}>{d.day}</span>
                      )}
                    </div>
          ...
          <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {monthNames.map((monthName, index) => (
              <button
                ...
                className={`p-6 bg-[#121c27] rounded-2xl border transition-all text-center group
                  ${index + 1 === month ? 'border-brand-500 shadow-md ring-2 ring-brand-500/20' : 'border-white/10 shadow-sm hover:border-brand-300 hover:shadow-md'}
                `}
              >
        ...
        {viewType === 'Día' && (
          <section className="bg-[#121c27] p-6 rounded-2xl border border-white/10/80 shadow-sm min-h-[400px]">
            ...
            {(!eventsByDate[activeDateStr] || eventsByDate[activeDateStr].length === 0) ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-lg font-bold text-slate-300">Sin datos programados</h3>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {event.amount && (
                          <span className="text-sm font-bold bg-[#121c27]/50 px-2 py-1 rounded-md">
