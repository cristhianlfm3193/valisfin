...
export function PaymentCard({ payment, onToggleStatus, onPartialPayment, onEdit, onViewHistory }: PaymentCardProps) {
  ...
  if (payment.isSmartCard) {
    ...
    return (
      <article className="payment-card bg-[#121c27] border border-white/10 rounded-2xl p-4 shadow-sm flex flex-col justify-between relative overflow-hidden">
        <div className="border-l-4 border-indigo-500 -ml-4 -mt-4 pl-4 pt-4 pb-1">
          <div className="flex justify-between items-start">
            <div>
              <span className="inline-block text-[11px] font-semibold text-indigo-400 uppercase tracking-wider">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                {onEdit && (
                  <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="p-1.5 text-slate-400 hover:text-indigo-400 hover:bg-white/5 rounded-lg transition-colors" title="Editar límite">
                    <Edit2 className="w-4 h-4" />
                  </button>
                )}
                {onViewHistory && (
                  <button onClick={(e) => { e.stopPropagation(); onViewHistory(); }} className="p-1.5 text-slate-400 hover:text-indigo-400 hover:bg-white/5 rounded-lg transition-colors" title="Ver historial">
        ...
        <div className="mt-3 pt-3 border-t border-white/10 flex flex-col gap-2">
          <div className="flex justify-between items-baseline">
            <span className="text-xs text-slate-400 font-medium">Gastado: <strong className="text-white">B/. {spent.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</strong></span>
            <span className="text-xs text-slate-400 font-medium">Límite: <strong className="text-white">{formattedAmount}</strong></span>
          </div>
                ...
                <button
                  ...
                  className={`pay-toggle-btn min-h-[44px] flex-1 px-3 py-2 rounded-xl text-xs font-bold tracking-wide transition-all flex items-center justify-center focus:outline-none ${
                    (payment.accumulatedSpent && payment.accumulatedSpent <= 0)
                      ...
                      : 'bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 focus:ring-2 focus:ring-indigo-500/50'
                  }`}
                  type="button"
                >
                  <span>Abonar</span>
              ...
              <button
                ...
                className={`pay-toggle-btn min-h-[44px] flex-[2] px-3 py-2 rounded-xl text-xs font-bold tracking-wide transition-all shadow-sm flex items-center justify-center gap-1.5 focus:outline-none ${
                  (payment.accumulatedSpent && payment.accumulatedSpent <= 0)
                    ...
                    : 'bg-emerald-600 hover:bg-emerald-500 text-white focus:ring-2 focus:ring-emerald-500'
                }`}
                type="button"
              >
