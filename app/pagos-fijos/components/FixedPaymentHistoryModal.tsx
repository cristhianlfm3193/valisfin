  ...
  const modalContent = (
    <>
      ...
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md z-[101] p-4">
        <div className="bg-[#121c27] rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
          {/* Header */}
          <div className="px-6 py-5 border-b border-white/5 flex items-center justify-between bg-[#121c27] shrink-0">
            ...
            <button 
              onClick={onClose}
              className="p-2 text-slate-400 hover:text-slate-400 hover:bg-white/5 rounded-full transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
              ...
              {historyPayments.length > 0 ? (
                <div className="space-y-3">
                  {historyPayments.map((item, idx) => (
                    <div 
                      key={item.id || idx}
                      className="bg-[#121c27] p-4 rounded-2xl border border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 hover:border-emerald-200 transition-colors shadow-sm"
                    >
                ...
                <div className="text-center py-10 bg-[#121c27] rounded-2xl border border-white/5 border-dashed">
