...
export function MobileNavigation({ user, profile }: { user?: User, profile?: any }) {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200/80 lg:hidden z-40 pb-safe shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)]">
        <div className="flex items-center justify-around px-2 py-1.5 mb-1">
          {bottomNavItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex flex-col items-center p-2 rounded-xl min-w-[72px] transition-colors ${isActive ? 'text-emerald-700' : 'text-slate-500 hover:text-slate-900'}`}
              >
                <div className={`p-1.5 rounded-lg mb-0.5 ${isActive ? 'bg-emerald-100/50' : ''}`}>
                  <Icon className={`w-6 h-6 ${isActive ? 'fill-emerald-100/50' : ''}`} />
                </div>
          ...
          <button
            ...
            className={`flex flex-col items-center p-2 rounded-xl min-w-[72px] transition-colors ${isMenuOpen ? 'text-emerald-700' : 'text-slate-500'}`}
          >
            <div className={`p-1.5 rounded-lg mb-0.5 ${isMenuOpen ? 'bg-emerald-100/50' : ''}`}>
