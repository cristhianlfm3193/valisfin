...
export default function AddMaintenanceModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-all duration-200">
      <div className="max-h-[90vh] overflow-y-auto custom-scrollbar bg-[#121c27] border-white/10 w-full max-w-lg rounded-3xl p-6 sm:p-7 shadow-2xl border border-outline-subtle transform scale-100 transition-all duration-200">
        <div className="flex items-center justify-between pb-4 border-b border-white/5">
          ...
          <button type="button" onClick={onClose} className="w-8 h-8 rounded-full bg-white/10 hover:bg-slate-200 text-gray-400 flex items-center justify-center transition-colors">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 pt-4">
          ...
          <div>
            <label className="block text-xs font-bold text-gray-300 mb-1.5">Vehículo</label>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-300 mb-1.5">Servicios frecuentes sugeridos</label>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-300 mb-1">Descripción del trabajo realizado</label>
            <input 
              name="service"
              type="text" 
              value={serviceDesc}
              ...
              className="w-full px-3.5 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-on-surface focus:outline-none focus:bg-[#121c27] border-white/10 focus:border-[#006655] transition-all" 
            />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-gray-300 mb-1">Kilometraje actual</label>
              <input 
                name="km"
                type="number" 
                defaultValue={initialData?.km_lectura || currentVehicleObj?.current_km || ''}
                required
                className="w-full px-3.5 py-2 bg-white/5 border border-white/10 rounded-xl text-sm font-mono text-on-surface focus:outline-none focus:bg-[#121c27] border-white/10 focus:border-[#006655] transition-all" 
              />
            </div>
            <div>
              ...
              <input 
                ...
                className="w-full px-3.5 py-2 bg-white/5 border border-white/10 rounded-xl text-sm font-mono text-on-surface focus:outline-none focus:bg-[#121c27] border-white/10 focus:border-[#006655] transition-all placeholder:text-slate-400 placeholder:text-xs" 
              />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-bold text-gray-300 mb-1">Costo (B/.)</label>
              <input 
                name="cost"
                type="number" 
                step="0.01"
                ...
                placeholder="0.00"
                className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-sm font-mono font-bold text-on-surface focus:outline-none focus:bg-[#121c27] border-white/10 focus:border-[#006655] transition-all" 
              />
            </div>
            <div>
              ...
              <input 
                ...
                className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-sm text-on-surface focus:outline-none focus:bg-[#121c27] border-white/10 focus:border-[#006655] transition-all" 
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-300 mb-1">Taller / Local</label>
              <input 
                name="shop"
                type="text" 
          ...
          <div className="flex items-center justify-end gap-2 pt-3 border-t border-white/5">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-full text-xs font-semibold text-gray-300 hover:bg-white/10 transition-colors">Cancelar</button>
