...
export default function AddKmModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-all duration-200">
      <div className="max-h-[90vh] overflow-y-auto custom-scrollbar bg-[#121c27] w-full max-w-md rounded-3xl p-6 shadow-2xl border border-outline-subtle transform scale-100 transition-all duration-200">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 pt-4">
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">Vehículo a actualizar</label>
            <select 
              name="vehicle_id"
              value={selectedVehicle}
              onChange={(e) => handleVehicleChange(e.target.value)}
              className="w-full px-3.5 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm font-semibold text-on-surface focus:outline-none focus:bg-[#121c27] focus:border-[#006655] transition-all"
            >
              {vehicles.map(v => (
          ...
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1">Nueva lectura de odómetro</label>
            <div className="relative">
              <input 
                ...
                required
                className="w-full pl-4 pr-14 py-3 bg-white/5 border border-white/10 rounded-xl font-mono text-2xl font-bold text-white focus:outline-none focus:bg-[#121c27] focus:border-[#006655] transition-all" 
              />
          ...
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">Conductor / Registrado por</label>
            <div className="grid grid-cols-2 gap-2">
              <label className="flex items-center gap-2 p-2 rounded-xl bg-white/5 border border-white/10 cursor-pointer">
                ...
                <span className="text-xs font-semibold text-slate-200">Cristhian Fuentes</span>
              </label>
              <label className="flex items-center gap-2 p-2 rounded-xl bg-white/5 border border-white/10 cursor-pointer">
                <input type="radio" name="user_id" value="7b5c62be-58f1-48d6-b366-0f504c39bdcb" className="text-[#006655] focus:ring-[#006655]" />
                <span className="text-xs font-semibold text-slate-200">Jennifer Camaño</span>
              </label>
            </div>
          </div>
