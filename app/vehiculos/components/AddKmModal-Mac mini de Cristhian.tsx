...
export default function AddKmModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-all duration-200">
      <div className="max-h-[90vh] overflow-y-auto custom-scrollbar bg-[#121c27] border-white/10 w-full max-w-md rounded-3xl p-6 shadow-2xl border border-outline-subtle transform scale-100 transition-all duration-200">
        <div className="flex items-center justify-between pb-4 border-b border-white/5">
          ...
          <button onClick={onClose} type="button" className="w-8 h-8 rounded-full bg-white/10 hover:bg-slate-200 text-gray-400 flex items-center justify-center transition-colors">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 pt-4">
          <div>
            <label className="block text-xs font-bold text-gray-300 mb-1.5">Vehículo a actualizar</label>
            <select 
              name="vehicle_id"
              value={selectedVehicle}
              onChange={(e) => handleVehicleChange(e.target.value)}
              className="w-full px-3.5 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm font-semibold text-on-surface focus:outline-none focus:bg-[#121c27] border-white/10 focus:border-[#006655] transition-all"
            >
              {vehicles.map(v => (
          ...
          <div>
            <label className="block text-xs font-bold text-gray-300 mb-1">Nueva lectura de odómetro</label>
            <div className="relative">
              <input 
                ...
                required
                className="w-full pl-4 pr-14 py-3 bg-white/5 border border-white/10 rounded-xl font-mono text-2xl font-bold text-white focus:outline-none focus:bg-[#121c27] border-white/10 focus:border-[#006655] transition-all" 
              />
          ...
          <div>
            <label className="block text-xs font-bold text-gray-400 mb-1.5">Incremento rápido sugerido</label>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-300 mb-1.5">Conductor / Registrado por</label>
            <div className="grid grid-cols-2 gap-2">
              <label className="flex items-center gap-2 p-2 rounded-xl bg-white/5 border border-white/10 cursor-pointer">
                ...
                <span className="text-xs font-semibold text-white">Cristhian Fuentes</span>
              </label>
              <label className="flex items-center gap-2 p-2 rounded-xl bg-white/5 border border-white/10 cursor-pointer">
                <input type="radio" name="user_id" value="7b5c62be-58f1-48d6-b366-0f504c39bdcb" className="text-[#006655] focus:ring-[#006655]" />
                <span className="text-xs font-semibold text-white">Jennifer Camaño</span>
              </label>
            </div>
          </div>
