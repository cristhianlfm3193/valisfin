      ...
      <div className="max-w-7xl mx-auto w-full space-y-6 lg:space-y-8">
        ...
        <section className="flex flex-col sm:flex-row sm:items-center justify-between gap-4" data-purpose="page-title-banner">
          <div>
            ...
            <h1 className="text-3xl lg:text-4xl font-black text-white tracking-tight">Metas de Ahorro</h1>
            <p className="text-xs sm:text-sm text-gray-400 mt-1">Planificación y seguimiento de compras, mejoras del hogar y objetivos personales.</p>
          </div>
          <AddGoalModal />
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-5" data-purpose="bento-kpi-summary">
          {/* KPI 1 */}
          <article className="bg-[#121c27] border-white/10 rounded-2xl p-5 border border-white/10/80 shadow-xs relative overflow-hidden group hover:border-slate-300 transition-all">
            ...
            <p className="text-2xl sm:text-3xl font-black text-white tracking-tight">B/. {formatCurrency(globalTarget)}</p>
            <div className="mt-2.5 flex items-center gap-1.5 text-xs text-gray-400 font-medium">
          ...
          {/* KPI 2 */}
          <article className="bg-[#121c27] border-white/10 rounded-2xl p-5 border border-brand-200/80 shadow-xs relative overflow-hidden group bg-gradient-to-b from-white to-brand-50/20">
          ...
          <article className="bg-[#121c27] border-white/10 rounded-2xl p-5 border border-white/10/80 shadow-xs relative overflow-hidden group hover:border-slate-300 transition-all">
            ...
            <p className="text-2xl sm:text-3xl font-black text-white tracking-tight">B/. {formatCurrency(globalMissing)}</p>
            <div className="mt-2.5 flex items-center gap-1.5 text-xs text-gray-400 font-medium">
        ...
        {/* Search */}
        <section className="bg-[#121c27] border-white/10 p-3.5 sm:p-4 rounded-2xl border border-white/10/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 bg-white/5/70 border border-white/10 text-xs sm:text-sm rounded-xl focus:bg-[#121c27] border-white/10 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all placeholder:text-slate-400 font-medium" 
        ...
        {/* Table */}
        <section className="bg-[#121c27] border-white/10 rounded-2xl border border-white/10/80 shadow-[0_4px_12px_rgba(0,0,0,0.5)] overflow-hidden w-full max-w-full">
          <div className="hidden lg:block overflow-x-auto w-full">
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="border-b border-white/5 bg-white/5/50 text-xs font-bold text-slate-400 uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredGoals.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="py-8 text-center text-gray-400">No se encontraron metas de ahorro.</td>
                  </tr>
                  ...
                  return (
                    <tr key={goal.id} className="hover:bg-white/5/50 transition-colors">
                      ...
                      <td className="py-4 px-5 font-semibold text-gray-300 whitespace-nowrap">B/. {formatCurrency(goal.target_amount)}</td>
                      <td className="py-4 px-5 font-bold text-brand-700 whitespace-nowrap">B/. {formatCurrency(goal.saved_amount)}</td>
                      <td className="py-4 px-5 font-semibold text-gray-400 whitespace-nowrap">B/. {formatCurrency(missing)}</td>
                      <td className="py-4 px-5 whitespace-nowrap">
          ...
          <div className="block lg:hidden divide-y divide-slate-100">
            {filteredGoals.map(goal => {
              ...
              return (
                <div key={goal.id} className="p-4 sm:p-5 space-y-3 hover:bg-white/5/50 transition-colors">
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="font-bold text-white leading-tight">{goal.title}</h3>
                      <p className="text-[11px] font-medium text-gray-400 mt-0.5">{goal.category}</p>
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-bold text-gray-400 w-8">{progress.toFixed(0)}%</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-xs pt-1">
                    <div>
                      <span className="block text-[10px] text-slate-400 uppercase tracking-wider mb-0.5 font-bold">Costo</span>
                      <span className="font-semibold text-gray-300">B/.{formatCurrency(goal.target_amount)}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="font-semibold text-gray-400">B/.{formatCurrency(missing)}</span>
                    </div>
                  </div>
