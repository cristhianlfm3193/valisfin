...
const MapaLocalesClient = dynamic(() => import('./MapaLocalesClient'), {
  ssr: false,
  loading: () => (
    <div className="w-full h-96 flex items-center justify-center bg-[#121c27]/85 backdrop-blur-md rounded-2xl border border-white/10">
      <div className="flex flex-col items-center gap-3">
        <div className="w-8 h-8 border-4 border-pink-500 border-t-transparent rounded-full animate-spin"></div>
        <span className="text-gray-400 font-semibold text-sm">Cargando inteligencia de campo...</span>
      </div>
    </div>
  )
});
