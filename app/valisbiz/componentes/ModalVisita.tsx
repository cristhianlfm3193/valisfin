...
export default function ModalVisita({ onClose, locales, vendedores, localInicial, visitaAEditar }: ModalVisitaProps) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[999] flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-[#181124] border border-white/10 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        
        <div className="flex items-center justify-between p-5 border-b border-white/5">
          <h3 className="font-bold text-lg text-gray-100">
            {isEditing ? 'Editar Visita' : 'Registrar Visita'}
          </h3>
          <button onClick={onClose} className="p-2 text-gray-400 hover:text-gray-300 hover:bg-[#090a0f]/50/10 rounded-full transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-bold text-gray-300">Fecha de Visita</label>
            <input 
              type="date" 
              required
              value={fecha}
              onChange={e => setFecha(e.target.value)}
              className="w-full bg-[#090a0f]/50/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm font-medium text-gray-100 focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 transition-all"
            />
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-bold text-gray-300">Vendedor</label>
            <select 
              required
              value={vendedorId}
              onChange={e => {
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-bold text-gray-300">Tipo de Local</label>
            <select 
              value={tipoLocal}
              onChange={e => {
          ...
          <div className="flex flex-col gap-1.5 relative">
            <label className="text-sm font-bold text-gray-300">Punto de Venta (Local)</label>
            <input 
              required
              type="text"
              value={searchLocal}
            ...
            />
            {showDropdown && (
              <div className="absolute top-full left-0 right-0 mt-1 max-h-48 overflow-y-auto bg-[#090a0f]/50 border border-white/10 rounded-xl shadow-lg z-50">
                {locales.filter(l => {
                  ...
                  <div 
                    key={l.id} 
                    className="px-4 py-2 hover:bg-pink-50 cursor-pointer text-sm font-medium text-gray-200"
                    onClick={() => {
                      setLocalId(l.id);
                  ...
                  >
                    {l.nombre_local} <span className="text-xs text-gray-400">({l.tipo})</span>
                  </div>
                ))}
                {locales.filter(l => {
                  if (l.activo === false) return false;
                  ...
                  <div className="px-4 py-3 text-sm text-gray-400 text-center">No se encontraron locales.</div>
                )}
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-bold text-gray-300">Resultado de la Visita</label>
            <div className="grid grid-cols-2 gap-3">
              <button 
                type="button"
                onClick={() => setEstadoVisita('con_compra')}
                className={`flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all ${estadoVisita === 'con_compra' ? 'border-emerald-500 bg-emerald-50 text-whitemerald-700' : 'border-white/5 bg-[#090a0f]/50 text-gray-400 hover:border-emerald-500/50'}`}
              >
                <CheckCircle2 className={`w-6 h-6 ${estadoVisita === 'con_compra' ? 'text-whitemerald-500' : 'text-gray-400'}`} />
