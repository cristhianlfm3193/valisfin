...
// Formato de fecha único y homologado para todas las tablas: "10 sept 2026"
function formatDateStandard(fechaIso: string): string {
  ...
  return `${dayNum.toString().padStart(2, '0')} ${monthStr} ${year}`;
}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 bg-white px-2.5 py-1.5 border border-slate-200 rounded-xl shadow-xs">
              ...
              <div className="flex items-center gap-1 text-xs">
                ...
                />
                <span className="text-slate-400 font-semibold ml-1">Hasta:</span>
                <input 
                  type="date" 
                  value={fechaHasta} 
...
function TablaFacturado({ registros, onRefresh }: { registros: RegistroFacturadoFila[]; onRefresh: () => void }) {
  ...
  const filtrados = useMemo(() => {
    return registros.filter(r => {
      ...
      // 2. Filtro Vendedor
      if (vendedorFiltro !== 'todos' && !r.vendedor_nombre.toLowerCase().includes(vendedorFiltro.toLowerCase())) {
        return false;
      }
      // 3. Filtro Fecha Desde (YYYY-MM-DD)
      if (fechaDesde && r.fecha < fechaDesde) {
...
function TablaComparativaDiaria({ registrosFacturado, registrosVendido }: {
  ...
  const comparativa = useMemo(() => {
    ...
    // 2. Filtro Vendedor
    if (vendedorFiltro !== 'todos') {
      filas = filas.filter(r => r.vendedor.toLowerCase().includes(vendedorFiltro.toLowerCase()));
    }
