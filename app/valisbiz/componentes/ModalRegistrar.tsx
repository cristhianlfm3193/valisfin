...
import { X, ChevronLeft, ChevronRight, Search, Building2, FileText } from 'lucide-react';
...
export default function ModalRegistrar({ vendedores, onClose, onSuccess }: ModalRegistrarProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      let result;
      if (tab === 'facturado') {
        result = await registrarFacturado(vendedorId, montoNum, new Date(fecha + 'T12:00:00'), notas);
      } else {
        result = await registrarVenta(vendedorId, montoNum, undefined, new Date(fecha + 'T12:00:00'));
      }

      if (result.success) {
        setSuccess(tab === 'facturado'
          ? '✅ Facturación registrada correctamente.'
          : '✅ Venta reportada registrada correctamente.');
        reset();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="bg-gradient-to-r from-pink-600 to-rose-500 px-6 py-4 flex items-center justify-between">
          <div>
        ...
        {/* Context note */}
        <div className={`px-6 py-2.5 text-[11px] font-medium ${tab === 'facturado' ? 'bg-pink-50 text-pink-700 border-b border-pink-100' : 'bg-blue-50 text-blue-700 border-b border-blue-100'}`}>
          {tab === 'facturado'
            ? '📊 Ingresa el acumulado que Finanzas reporta. Se guardará el monto del día y la tabla mostrará el último registro.'
            : '📋 Ingresa lo que el vendedor reportó. La tabla sumará todos los registros del mes de forma acumulativa.'}
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">
              {tab === 'facturado' ? 'Monto Facturado (B/.) *' : 'Monto Vendido del Día (B/.) *'}
            </label>
          ...
          {tab === 'facturado' && (
            <div>
              ...
              <input
                ...
                placeholder="Ej: Reporte diario finanzas 09/09/2026"
                className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-pink-300 focus:border-pink-400 transition-all"
              />
            </div>
          )}
