      ...
      <div className="relative w-full max-w-xl bg-[#121c27] rounded-2xl shadow-2xl border border-white/5 overflow-hidden my-auto flex flex-col max-h-[92vh]">
        <div className="px-6 py-5 border-b border-white/5 flex items-start justify-between bg-white/5/50 shrink-0">
          <div className="space-y-1">
            <div>
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-800 uppercase tracking-wider">
        ...
        <form action={formAction} className="flex-1 flex flex-col overflow-hidden">
          <div className="p-5 sm:p-6 overflow-y-auto space-y-4 flex-1">
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Área / Ubicación del Hogar</label>
              <select name="area" required defaultValue={initialData?.hogar_area || "Sala / Comedor"} className="w-full text-xs sm:text-sm rounded-xl border border-white/10 bg-[#121c27] px-3.5 py-2.5 text-slate-200 font-medium focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20 shadow-sm">
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Tipo de Trabajo / Mantenimiento</label>
              <input name="title" required type="text" defaultValue={initialData?.detalle || ''} placeholder="Ej. Limpieza profunda de Aire Acondicionado Inverter" className="w-full text-xs sm:text-sm rounded-xl border border-white/10 px-3.5 py-2.5 text-slate-200 font-medium placeholder-slate-400 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20 shadow-sm" />
            </div>

            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Detalle / Notas Técnicas</label>
              <textarea name="description" placeholder="Detalles de repuestos, medidas o alcance..." rows={2} className="w-full text-xs sm:text-sm rounded-xl border border-white/10 px-3.5 py-2 text-slate-200 font-medium placeholder-slate-400 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20 shadow-sm resize-none"></textarea>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Fecha Estimada</label>
                <input name="estimated_date" type="date" required defaultValue={initialData?.fecha || ''} className="w-full text-xs sm:text-sm rounded-xl border border-white/10 px-3.5 py-2 text-slate-200 font-medium focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20 shadow-sm" />
              </div>
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Presupuesto Estimado</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Responsable Familiar</label>
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Prioridad / Estado</label>
                <select name="priority" required defaultValue={initialData?.hogar_prioridad || "Media"} className="w-full text-xs sm:text-sm rounded-xl border border-white/10 bg-[#121c27] px-3.5 py-2 text-slate-200 font-medium focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20 shadow-sm">
