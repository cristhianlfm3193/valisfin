    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-all duration-200">
      <div className="max-h-[90vh] overflow-y-auto custom-scrollbar bg-[#121c27] border-white/10 w-full max-w-md rounded-3xl p-6 shadow-2xl border border-white/10 transform scale-100 transition-all duration-200">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 pt-4">
          <div>
            <label className="block text-xs font-bold text-gray-300 mb-1.5">Fecha del Mantenimiento</label>
            <input 
              name="maintenance_date"
              type="date" 
              required
              defaultValue={today}
              className="w-full px-3.5 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm font-semibold text-white focus:outline-none focus:bg-[#121c27] border-white/10 focus:border-blue-600 transition-all" 
            />
          </div>

          <div>
            <label className="block text-xs font-bold text-gray-300 mb-1.5">Costo (B/.)</label>
            <input 
              name="cost"
              type="number" 
              step="0.01"
          ...
          <div>
            <label className="block text-xs font-bold text-gray-300 mb-1.5">Técnico / Empresa (Opcional)</label>
            <input 
              name="technician"
              type="text" 
          ...
          <div className="flex items-center justify-end gap-2 pt-4 border-t border-white/5">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-xl text-xs font-bold text-gray-300 hover:bg-white/10 transition-colors">Cancelar</button>
            <button type="submit" disabled={loading} className="px-5 py-2 rounded-xl text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-[0_8px_24px_rgba(0,0,0,0.6)] shadow-blue-900/10 transition-all disabled:opacity-50">
