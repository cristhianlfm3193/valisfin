...
export default function EditHomeTaskModal({ task, isOpen, onClose }: { task: any, isOpen: boolean, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/50 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-xl bg-[#121c27] rounded-2xl shadow-2xl border border-white/5 overflow-hidden my-auto flex flex-col max-h-[92vh]">
        ...
        <form action={formAction} className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="p-5 sm:p-6 overflow-y-auto space-y-4 flex-1">
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Área / Ubicación</label>
              <select name="area" required defaultValue={task.area} className="w-full text-xs sm:text-sm rounded-xl border border-white/10 bg-[#121c27] px-3.5 py-2.5 text-slate-200 font-medium focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 shadow-sm">
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Tipo de Trabajo</label>
              <input name="title" required type="text" defaultValue={task.title} className="w-full text-xs sm:text-sm rounded-xl border border-white/10 px-3.5 py-2.5 text-slate-200 font-medium placeholder-slate-400 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 shadow-sm" />
            </div>

            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Detalles</label>
              <textarea name="description" defaultValue={task.description || ''} rows={2} className="w-full text-xs sm:text-sm rounded-xl border border-white/10 px-3.5 py-2 text-slate-200 font-medium placeholder-slate-400 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 shadow-sm resize-none"></textarea>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Fecha Estimada</label>
                <input name="estimated_date" type="date" required defaultValue={task.estimated_date ? task.estimated_date.split('T')[0] : ''} className="w-full text-xs sm:text-sm rounded-xl border border-white/10 px-3.5 py-2 text-slate-200 font-medium focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 shadow-sm" />
              </div>
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Presupuesto / Costo Final</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Responsable</label>
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Estado Actual</label>
                <select name="status" required defaultValue={task.status} className="w-full text-xs sm:text-sm rounded-xl border border-white/10 bg-[#121c27] px-3.5 py-2 text-slate-200 font-medium focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 shadow-sm">
