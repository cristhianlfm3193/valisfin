...
export default function AddACMaintenanceModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-all duration-200">
      <div className="max-h-[90vh] overflow-y-auto custom-scrollbar bg-[#121c27] w-full max-w-md rounded-3xl p-6 shadow-2xl border border-white/10 transform scale-100 transition-all duration-200">
        <div className="flex items-center justify-between pb-4 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-lg font-extrabold text-white">{unit.location}</h3>
            </div>
          </div>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 pt-4">
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">Fecha del Mantenimiento</label>
            <input 
              name="maintenance_date"
              type="date" 
              required
              defaultValue={today}
              className="w-full px-3.5 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm font-semibold text-white focus:outline-none focus:bg-[#121c27] focus:border-blue-600 transition-all" 
            />
          </div>

          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">Costo (B/.)</label>
            <input 
              name="cost"
              type="number" 
              step="0.01"
          ...
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">Técnico / Empresa (Opcional)</label>
            <input 
              name="technician"
              type="text" 
          ...
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">Responsable / Pagado por</label>
            <div className="grid grid-cols-2 gap-2">
              <label className="flex items-center gap-2 p-2 rounded-xl bg-white/5 border border-white/10 cursor-pointer">
                ...
                <span className="text-xs font-semibold text-slate-200">Cristhian</span>
              </label>
              <label className="flex items-center gap-2 p-2 rounded-xl bg-white/5 border border-white/10 cursor-pointer">
                <input type="radio" name="profile_id" value="7b5c62be-58f1-48d6-b366-0f504c39bdcb" className="text-pink-600 focus:ring-pink-600" />
                <span className="text-xs font-semibold text-slate-200">Jennifer</span>
              </label>
            </div>
          </div>

          <div className="flex items-center justify-end gap-2 pt-4 border-t border-white/5">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-xl text-xs font-bold text-slate-400 hover:bg-slate-100 transition-colors">Cancelar</button>
