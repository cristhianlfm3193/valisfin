            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-slate-100 text-[11px] font-bold text-slate-400 uppercase tracking-wider bg-slate-50/50">
                  <th className="py-3 px-5">Registro</th>
                  <th className="py-3 px-5">Fecha Est.</th>
                  <th className="py-3 px-5">Área</th>
                  <th className="py-3 px-5">Trabajo</th>
                  <th className="py-3 px-5 text-center">Estado</th>
                  <th className="py-3 px-5 text-right">Presupuesto</th>
                  <th className="py-3 px-5">Asignado</th>
                  <th className="py-3 px-5 text-center">Acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                {filteredTasks.length === 0 ? (
                  <tr><td colSpan={8} className="py-8 text-center text-slate-500">No hay tareas para mostrar.</td></tr>
                ) : filteredTasks.map(task => (
                  <tr key={task.id} className={`hover:bg-slate-50/70 transition-colors ${task.status === 'Completado' ? 'bg-emerald-50/20' : ''}`}>
                    <td className="py-4 px-5 text-slate-500 whitespace-nowrap">{formatDate(task.registration_date)}</td>
                    <td className="py-4 px-5 whitespace-nowrap">
                      {task.status === 'Completado' ? (
                        ...
                        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-xs font-semibold">
                          <Clock className="w-3.5 h-3.5" />
                          <span>{formatDate(task.estimated_date)}</span>
                        </div>
                      )}
                    </td>
                    <td className="py-4 px-5 whitespace-nowrap">
                    ...
                    <td className="py-4 px-5 font-semibold text-slate-800 max-w-xs">
                      {task.title}
                      {task.description && <span className="block text-[11px] font-normal text-slate-400 mt-0.5 truncate">{task.description}</span>}
                    </td>
                    ...
                    <td className="py-4 px-5 text-right font-bold text-slate-900 whitespace-nowrap">B/. {formatCurrency(task.budget || 0)}</td>
                    <td className="py-4 px-5 whitespace-nowrap">
                    ...
                    <td className="py-4 px-5 text-center whitespace-nowrap">
                      <div className="inline-flex items-center justify-center gap-1.5">
                        {task.status !== 'Completado' && (
                          <button onClick={() => handleComplete(task.id)} className="w-7 h-7 rounded-lg border border-emerald-200 text-emerald-600 hover:bg-emerald-50 flex items-center justify-center transition-colors" title="Marcar como cumplido">
                            <CheckCircle2 className="w-4 h-4" />
                          </button>
                        )}
                        <button onClick={() => setEditingTask(task)} className="w-7 h-7 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 flex items-center justify-center transition-colors" title="Editar trabajo">
                          <PenSquare className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => handleDelete(task.id)} className="w-7 h-7 rounded-lg border border-red-200 text-red-500 hover:bg-red-50 flex items-center justify-center transition-colors" title="Eliminar registro">
