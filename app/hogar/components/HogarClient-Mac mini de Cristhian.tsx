  ...
  return (
    <>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-2.5 sm:gap-3 w-full sm:w-auto">
            <button 
              ...
              className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-3.5 py-2.5 text-xs font-semibold rounded-xl bg-[#121c27] border-white/10 border border-white/10 text-gray-300 hover:bg-white/5 transition-colors shadow-[0_4px_12px_rgba(0,0,0,0.5)] min-h-[44px]"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5">
          ...
          <div className="bg-[#121c27] border-white/10 rounded-2xl border border-white/10/80 p-5 shadow-[0_4px_12px_rgba(0,0,0,0.5)] relative overflow-hidden group hover:border-emerald-200 transition-all">
            ...
            <div className="mt-2.5">
              ...
              <p className="text-xs text-gray-400 mt-1">{nextRevision ? `${nextRevision.area} • Programado para ${formatDate(nextRevision.estimated_date || nextRevision.registration_date)}` : "No hay tareas pendientes."}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <main className="px-4 sm:px-6 lg:px-8 py-3 sm:py-4 pb-24 lg:pb-6 flex-1">
        <div className="bg-[#121c27] border-white/10 rounded-2xl border border-white/10/90 shadow-[0_4px_12px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col">
          <div className="p-5 border-b border-white/5 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-base font-bold text-white">Trabajos y Reparaciones del Hogar</h2>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative w-full sm:w-auto">
                ...
                <input 
                  ...
                  className="pl-9 pr-3 py-2 text-xs rounded-xl border border-white/10 text-gray-300 placeholder-slate-400 focus:ring-emerald-600 focus:border-emerald-600 w-full sm:w-64 min-h-[40px]" 
              ...
              <div className="hidden md:flex items-center p-1 bg-white/10/80 rounded-xl border border-white/10/80 gap-1">
                {['Todos', 'Pendientes', 'Completados', 'Urgentes'].map(f => (
                  <button 
                    key={f}
                    onClick={() => setFilter(f)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 font-semibold rounded-lg text-xs transition-all ${filter === f ? 'bg-[#09574a] text-white shadow-[0_4px_12px_rgba(0,0,0,0.5)]' : 'text-gray-300 hover:text-white hover:bg-[#121c27] border-white/10/60'}`}
                  >
                    <span>{f}</span>
                  </button>
                ))}
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-slate-100 font-medium text-gray-300">
                {paginatedTasks.length === 0 ? (
                  <tr><td colSpan={5} className="py-8 text-center text-gray-400">No hay tareas para mostrar.</td></tr>
                  ...
                  <tr key={task.id} className={`hover:bg-white/5/70 transition-colors ${task.status === 'Completado' ? 'bg-emerald-50/20' : ''}`}>
                    ...
                    <td className="py-4 px-5">
                      <div className="flex flex-col gap-1">
                        ...
                        {task.description && <span className="text-[11px] font-normal text-gray-400 line-clamp-1">{task.description}</span>}
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-5 text-center whitespace-nowrap">
                      <div className="inline-flex items-center justify-center gap-1.5">
                        ...
                        <button onClick={() => setEditingTask(task)} className="w-8 h-8 rounded-lg border border-white/10 text-gray-300 hover:bg-white/5 flex items-center justify-center transition-colors shadow-[0_4px_12px_rgba(0,0,0,0.5)]" title="Editar trabajo">
                          <PenSquare className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDelete(task.id)} className="w-8 h-8 rounded-lg border border-red-200 text-red-500 hover:bg-red-50 flex items-center justify-center transition-colors shadow-[0_4px_12px_rgba(0,0,0,0.5)]" title="Eliminar registro">
          ...
          <div className="block md:hidden divide-y divide-slate-100">
             {paginatedTasks.length === 0 ? (
               <div className="py-8 text-center text-gray-400 text-xs">No hay tareas para mostrar.</div>
             ) : paginatedTasks.map(task => (
                <div key={task.id} className={`p-4 hover:bg-white/5/70 transition-colors space-y-3 ${task.status === 'Completado' ? 'bg-emerald-50/20' : ''}`}>
                  ...
                  <div className="flex items-center justify-between text-xs pt-1 border-t border-slate-50">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setEditingTask(task)} className="w-9 h-9 rounded-xl border border-white/10 text-gray-300 hover:bg-white/5 flex items-center justify-center transition-colors">
                        <PenSquare className="w-4 h-4" />
                      </button>
            ...
            <div className="flex items-center justify-between p-4 border-t border-white/5 bg-white/5/50">
              ...
              <div className="flex items-center gap-1">
                <button 
                  ...
                  className="px-3 py-1.5 rounded-lg border border-white/10 bg-[#121c27] border-white/10 text-xs font-semibold text-gray-300 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white/5 transition-colors"
                >
                  Anterior
                </button>
