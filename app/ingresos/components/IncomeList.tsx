...
export function IncomeList({ incomes, monthName = 'del Mes' }: { incomes: any[], monthName?: string }) {
  ...
  const renderBadge = (pendingCount: number, totalCount: number) => {
    ...
    return (
      <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-white/10 text-gray-300 font-semibold text-xs w-fit border border-white/10">
  ...
  const renderItem = (item: any) => {
    ...
    return (
      <div 
        key={item.id} 
        className={`bg-[#121c27] border-white/10 rounded-xl p-4 sm:p-5 border shadow-[0_4px_12px_rgba(0,0,0,0.5)] transition hover:shadow-[0_8px_24px_rgba(0,0,0,0.6)] flex flex-col md:flex-row md:items-center justify-between gap-4 ${isPending ? 'opacity-70 pointer-events-none' : ''} ${isReceived ? 'border-emerald-200/60' : 'border-white/10/80'}`}
      >
        <div className="flex items-start sm:items-center gap-3.5">
          <div className={`w-11 h-11 rounded-xl border flex items-center justify-center shrink-0 font-bold text-sm ${
            isCristhian ? 'bg-white/5 border-white/5 text-gray-300' : 'bg-pink-50 border-pink-100 text-pink-600'
          }`}>
            {initials}
          </div>
          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="font-semibold text-white text-sm sm:text-base">{item.description}</h3>
              <span className={`px-2 py-0.5 rounded-full font-semibold text-[10px] sm:text-xs ${
                isCristhian ? 'bg-white/10 text-gray-300' : 'bg-pink-100 text-pink-700'
              }`}>{personName}</span>
        ...
        <div className="flex items-center justify-between w-full md:w-auto gap-4 md:gap-6 mt-3 md:mt-0 pt-3 md:pt-0 border-t md:border-0 border-white/5/80 shrink-0">
          <div className="text-left md:text-right">
            <div className={`text-lg sm:text-xl font-bold font-mono whitespace-nowrap ${isReceived ? 'text-emerald-700' : 'text-white'}`}>
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-1">
        <button 
          ...
          className="w-full flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-2 py-2 rounded-xl hover:bg-white/5 transition text-left group"
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-white group-hover:text-emerald-700 transition-colors">Primera Quincena (1 al 15 de {monthName})</h2>
              <p className="text-xs sm:text-sm text-gray-400">Pagos de inicio de mes, colegiatura y primera ronda de compromisos familiares.</p>
            </div>
          </div>
          <div className="flex items-center gap-3 self-end sm:self-auto mt-2 sm:mt-0">
            {renderBadge(q1PendingCount, q1Items.length)}
            <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-gray-400 group-hover:bg-slate-200 transition">
          ...
          <div className="space-y-3 pt-2">
            ...
            {q1Items.length > ITEMS_LIMIT && (
              <button 
                ...
                className="w-full py-2.5 rounded-xl border border-dashed border-slate-300 text-gray-400 font-semibold text-sm hover:bg-white/5 hover:text-gray-300 hover:border-slate-400 transition"
              >
      ...
      <div className="space-y-1 pt-4">
        ...
        >
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-full bg-slate-200 text-gray-300 flex items-center justify-center font-bold text-sm shrink-0">2</div>
            <div>
              <h2 className="text-base sm:text-lg font-bold text-white group-hover:text-emerald-700 transition-colors">Segunda Quincena (16 al 30/31 de {monthName})</h2>
              <p className="text-xs sm:text-sm text-gray-400">Cierres de mes, servicios del hogar, ahorros y amortizaciones.</p>
            </div>
          </div>
          <div className="flex items-center gap-3 self-end sm:self-auto mt-2 sm:mt-0">
