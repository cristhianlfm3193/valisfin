...
export function IncomeList({ incomes, monthName = 'del Mes' }: { incomes: any[], monthName?: string }) {
  ...
  const renderBadge = (pendingCount: number, totalCount: number) => {
    ...
    return (
      <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-slate-100 text-slate-400 font-semibold text-xs w-fit border border-white/10">
  ...
  const renderItem = (item: any) => {
    ...
    return (
      <div 
        key={item.id} 
        className={`bg-[#121c27] rounded-xl p-4 sm:p-5 border shadow-sm transition hover:shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4 ${isPending ? 'opacity-70 pointer-events-none' : ''} ${isReceived ? 'border-emerald-200/60' : 'border-white/10/80'}`}
      >
        <div className="flex items-start sm:items-center gap-3.5">
          <div className={`w-11 h-11 rounded-xl border flex items-center justify-center shrink-0 font-bold text-sm ${
            isCristhian ? 'bg-white/5 border-white/5 text-slate-300' : 'bg-pink-50 border-pink-100 text-pink-600'
          }`}>
            {initials}
          </div>
          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`px-2 py-0.5 rounded-full font-semibold text-[10px] sm:text-xs ${
                isCristhian ? 'bg-slate-100 text-slate-400' : 'bg-pink-100 text-pink-700'
              }`}>{personName}</span>
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="space-y-3 pt-2">
            ...
            {q1Items.length > ITEMS_LIMIT && (
              <button 
                ...
                className="w-full py-2.5 rounded-xl border border-dashed border-slate-300 text-slate-500 font-semibold text-sm hover:bg-white/5 hover:text-slate-300 hover:border-slate-400 transition"
              >
      ...
      <div className="space-y-1 pt-4">
        ...
        >
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-full bg-slate-200 text-slate-300 flex items-center justify-center font-bold text-sm shrink-0">2</div>
            <div>
