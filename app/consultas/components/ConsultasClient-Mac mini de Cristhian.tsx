...
export default function ConsultasClient({ initialTransactions }: ConsultasClientProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto w-full space-y-6 lg:space-y-8 pb-10">
      ...
      <div className="flex flex-col gap-6 bg-[#121c27] border-white/10 p-5 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.5)] border border-white/10/80">
        
        {/* Omni-Search Bar */}
        <div className="relative flex items-center w-full">
          <Search className="absolute left-4 text-slate-400 w-5 h-5" />
          <input 
            className="w-full pl-12 pr-12 py-3.5 bg-white/5 border border-white/10 rounded-xl text-sm font-semibold text-white placeholder:text-slate-400 placeholder:font-normal focus:outline-none focus:bg-[#121c27] border-white/10 focus:border-brand-500 focus:ring-2 focus:ring-brand-500 transition-all" 
          ...
          {searchTerm && (
            <button 
              ...
              className="absolute right-4 p-1 rounded-md text-slate-400 hover:text-gray-300 transition-colors" 
        ...
        <div className="flex flex-wrap items-center gap-2.5 pt-1">
          {modulesList.map(mod => (
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold transition-all shadow-[0_4px_12px_rgba(0,0,0,0.5)] ${
                activeModule === mod 
                  ? 'bg-brand-600 text-white' 
                  : 'bg-white/10 text-gray-300 hover:bg-slate-200 border border-white/10'
              }`}
            >
              <span>{mod}</span>
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                activeModule === mod ? 'bg-brand-700/50 text-white' : 'bg-[#121c27] border-white/10 text-gray-300 border border-white/10'
              }`}>
                {moduleCounts[mod] || 0}
              </span>
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 pt-3">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Responsable</label>
            <select 
              value={responsibleFilter}
              onChange={(e) => setResponsibleFilter(e.target.value)}
              className="px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm font-semibold text-gray-300 focus:outline-none focus:ring-2 focus:ring-brand-500 cursor-pointer"
            >
          ...
          <div className="flex flex-col gap-1.5 lg:col-span-2">
            <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Fechas (Desde - Hasta)</label>
            <div className="flex items-center gap-2">
              <input 
                type="date"
                value={startDateFilter}
                onChange={(e) => setStartDateFilter(e.target.value)}
                className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm font-semibold text-gray-300 focus:outline-none focus:ring-2 focus:ring-brand-500 cursor-pointer"
              />
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Estatus</label>
            <select 
              value={statusFilter}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Método de Pago</label>
            <select 
          ...
          <div className="flex items-end">
            <button 
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 hover:bg-slate-200 text-gray-300 text-sm font-bold transition-colors border border-white/10"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-[#121c27] border-white/10 border border-white/10/80 shadow-xs flex flex-col justify-between gap-3 relative overflow-hidden group">
      ...
      <div className="flex flex-col bg-[#121c27] border-white/10 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.5)] border border-white/10/80 overflow-hidden">
        ...
        <div className="p-5 flex items-center justify-between flex-wrap gap-3 border-b border-white/5">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-white/10 text-xs text-gray-400 font-bold border border-white/10">
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100 text-sm text-gray-300">
              {filtered.map(t => {
                ...
                return (
                  <tr key={t.id + t.module} className="hover:bg-white/5/50 transition-colors group">
                    ...
                    <td className="py-4 px-5 font-mono text-xs text-gray-400 font-semibold whitespace-nowrap">{dateStr}</td>
                    <td className="py-4 px-5 font-bold text-white">{t.concept}</td>
                    <td className="py-4 px-5 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        {t.responsibleName !== 'Desconocido' ? (
                          <>
                            <span className={`w-6 h-6 rounded-full text-[10px] flex items-center justify-center font-bold text-white shadow-[0_4px_12px_rgba(0,0,0,0.5)] ${t.responsibleName === 'Cristhian' ? 'bg-brand-600' : t.responsibleName === 'Jennifer' ? 'bg-teal-600' : 'bg-slate-400'}`}>
