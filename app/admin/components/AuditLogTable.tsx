...
const DataViewer = ({ data }: { data: any }) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 bg-white/5 border border-white/5 rounded-2xl p-5 shadow-inner mt-2">
      {entries.map(([key, value]) => (
        <div key={key} className="flex flex-col">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{translateKey(key)}</span>
          <span className="text-sm font-medium text-slate-200 break-words">{formatValue(key, value)}</span>
        </div>
      ))}
    </div>
  );
...
export function AuditLogTable({ initialLogs }: AuditLogTableProps) {
  ...
  const getActionInfo = (action: string) => {
    switch (action) {
      ...
      case 'LOGOUT': return { label: 'CERRÓ SESIÓN', color: 'bg-slate-100 text-slate-300', icon: <Activity className="w-4 h-4 text-slate-400" /> };
      default: return { label: action, color: 'bg-slate-100 text-slate-300', icon: <Activity className="w-4 h-4 text-slate-400" /> };
    }
  };
  ...
  const getRecordDescription = (log: any) => {
    ...
    return (
      <span>
        {log.action === 'DELETE' ? 'Registro suprimido' : log.action === 'UPDATE' ? 'Modificación en' : 'Nuevo registro creado'}: <strong className="text-white">«{title}»</strong>
  ...
  return (
    <>
      <div className="bg-[#121c27] rounded-3xl border border-white/10 shadow-sm overflow-hidden mt-8">
        {/* Header */}
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 border-b border-white/5 bg-white/5/50 gap-4">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-2 border border-white/10 rounded-xl leading-5 bg-[#121c27] placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm transition-colors"
          ...
          {currentLogs.map((log) => {
            ...
            return (
              <div 
                ...
                className="p-5 hover:bg-white/5 transition-colors flex flex-col sm:flex-row gap-4 sm:items-center cursor-pointer group"
              >
                {/* Icon */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 text-xs mb-1.5">
                    ...
                    <span className="bg-slate-100 text-slate-400 px-2 py-0.5 rounded-md font-medium">
        ...
        {/* Pagination */}
        <div className="flex items-center justify-between p-4 sm:p-6 border-t border-white/5 bg-white/5/50">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="p-2 rounded-xl border border-white/10 bg-[#121c27] text-slate-400 hover:bg-white/5 disabled:opacity-50 disabled:hover:bg-[#121c27] transition-colors"
            >
      ...
      {selectedLog && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-[#121c27] rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border border-white/10 transform transition-all flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-6 border-b border-white/5 bg-white/5/50">
              <div>
                <h3 className="text-xl font-bold text-white">Detalles de Auditoría</h3>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-[#121c27] space-y-6">
              
              <div className="flex flex-wrap gap-4 text-sm bg-white/5 p-4 rounded-2xl border border-white/5">
                <div>
                ...
                <div>
                  ...
                  <span className="font-medium text-slate-300">{formatTableName(selectedLog.table_name)}</span>
                </div>
                <div>
                  <span className="block text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Fecha</span>
                  <span className="font-medium text-slate-300">{format(new Date(selectedLog.created_at), "PPP, p", { locale: es })}</span>
                </div>
                <div>
                  <span className="block text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Usuario</span>
                  <span className="font-medium text-slate-300">{selectedLog.profiles?.first_name || selectedLog.profiles?.email || 'Sistema'}</span>
                </div>
              </div>
